...
def _():
    ...
    m2 = df[df["id"].str.contains("UpperReservoir.Monitor2", case=False, na=False)].copy()
    ...
    if m2.empty:
        ...
        plt.title("Daily Out-of-Range Frequency — Monitor 2")
        plt.xlabel("Date")
        ...
        if daily_counts.sum() > 0:
            ...
            plt.title(f"Hourly Out-of-Range Frequency — {max_day.date()} — Monitor 2")
            plt.xlabel("Hour (UTC)")
