...
def _():
    ...
    out3_m1, low3_m1, up3_m1 = sigma_outliers(merged["lrl1"], 3)
    out5_m1, low5_m1, up5_m1 = sigma_outliers(merged["lrl1"], 5)
    out3_m2, low3_m2, up3_m2 = sigma_outliers(merged["lrl2"], 3)
    out5_m2, low5_m2, up5_m2 = sigma_outliers(merged["lrl2"], 5)

    #Plotting
    plt.figure(figsize=(15,6))
    plt.plot(merged.index, merged["lrl1"], color="cyan", alpha=0.7, label="Monitor 1")
    plt.plot(merged.index, merged["lrl2"], color="orange", alpha=0.7, label="Monitor 2")
    plt.scatter(merged.index[out3_m1], merged["lrl1"][out3_m1], color="red", s=10, label="3σ Outliers M1")
    plt.scatter(merged.index[out3_m2], merged["lrl2"][out3_m2], color="purple", s=10, label="3σ Outliers M2")
    ...
    # Null check
    null_m1 = merged["lrl1"].isna().sum()
    null_m2 = merged["lrl2"].isna().sum()
    ...
    print(f"Null % — M1: {merged['lrl1'].isna().mean()*100:.2f}% | M2: {merged['lrl2'].isna().mean()*100:.2f}%")
    ...
    # Operational range
    below1 = merged["lrl1"] < OPR_LOW
    above1 = merged["lrl1"] > OPR_HIGH
    below2 = merged["lrl2"] < OPR_LOW
    above2 = merged["lrl2"] > OPR_HIGH

    plt.figure(figsize=(15,6))
    plt.plot(merged.index, merged["lrl1"], color="cyan", alpha=0.8, label="Monitor 1")
    plt.plot(merged.index, merged["lrl2"], color="orange", alpha=0.8, label="Monitor 2")
    plt.fill_between(merged.index, OPR_LOW, OPR_HIGH, color="green", alpha=0.1, label="Normal Range")
    plt.scatter(merged.index[below1|above1], merged["lrl1"][below1|above1], color="red", s=10, label="Range Violations M1")
    plt.scatter(merged.index[below2|above2], merged["lrl2"][below2|above2], color="purple", s=10, label="Range Violations M2")
    plt.axhline(OPR_LOW, color="green", linestyle="--")
    plt.axhline(OPR_HIGH, color="red", linestyle="--")
    plt.title("Operational Range Violations (320.5–335.5 m)")
    plt.xlabel("Timestamp")
    plt.ylabel("Reservoir Level (m)")
    plt.legend()
    plt.grid(alpha=0.3)
