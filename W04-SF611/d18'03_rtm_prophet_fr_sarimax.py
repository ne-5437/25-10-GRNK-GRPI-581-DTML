...
@app.cell
def _(pd):
    df = pd.read_csv("RTM-MAY-TO-OCT.csv", parse_dates=["START_TIME"])
...
def _(
    ...
    y_true = test["ACTUAL"].values
    y_prop = prophet_pred.values
    y_sarx = sarima_fixed.values

    def rmse(a, b):
        return float(np.sqrt(mean_squared_error(a, b)))

    print("=== Evaluation Metrics (Weeks 41–43) ===\n")

    # Prophet Metrics
    print("Prophet:")
    print("  MAE  :", round(mean_absolute_error(y_true, y_prop), 3))
    print("  MSE  :", round(mean_squared_error(y_true, y_prop), 3))
    print("  RMSE :", round(rmse(y_true, y_prop), 3))
    print("  R²   :", round(r2_score(y_true, y_prop), 4))
    print()

    # SARIMAX (Fourier) Metrics
    print("SARIMA (Fourier-SARIMAX):")
    print("  MAE  :", round(mean_absolute_error(y_true, y_sarx), 3))
    print("  MSE  :", round(mean_squared_error(y_true, y_sarx), 3))
    print("  RMSE :", round(rmse(y_true, y_sarx), 3))
    print("  R²   :", round(r2_score(y_true, y_sarx), 4))
    print()

    # Metrics dictionary for export/reporting
    metrics = {
        "Prophet": {
            "MAE": round(mean_absolute_error(y_true, y_prop), 3),
            "MSE": round(mean_squared_error(y_true, y_prop), 3),
            "RMSE": round(rmse(y_true, y_prop), 3),
            "R2": round(r2_score(y_true, y_prop), 4)
        },
        "SARIMA_Fourier": {
            "MAE": round(mean_absolute_error(y_true, y_sarx), 3),
            "MSE": round(mean_squared_error(y_true, y_sarx), 3),
            "RMSE": round(rmse(y_true, y_sarx), 3),
            "R2": round(r2_score(y_true, y_sarx), 4)
        }
    }
