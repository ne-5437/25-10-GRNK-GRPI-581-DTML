...
def _(pd):
    # === Load CSV ===
    df = pd.read_csv("DAM-MAY-TO-OCT.csv", parse_dates=["START_TIME"]).sort_values("START_TIME")
...
def _(day_selector, get_filtered_data, mo, view_mode, week_selector):
    def compare_models_interactive():
        ...
        future_p = p_model.make_future_dataframe(periods=96, freq="15min")
        p_fc = p_model.predict(future_p)
        ...
        # SARIMA Forecast (same horizon as Prophet)
        sarima_pred = s_fit.get_forecast(steps=96)
        sarima_fc = sarima_pred.predicted_mean

        # Metrics (in-sample comparison only)
        y_true = d["ACTUAL"].values

        # Prophet fitted values (first len(d) rows)
        y_pred_p = p_fc["yhat"].iloc[:len(d)].values
        mae_p = mean_absolute_error(y_true, y_pred_p)
        rmse_p = rmse_metric(y_true, y_pred_p)

        # SARIMA fitted values (in-sample)
        y_pred_s = s_fit.fittedvalues.values
        mae_s = mean_absolute_error(y_true, y_pred_s)
        rmse_s = rmse_metric(y_true, y_pred_s)
        ...
        fig.add_trace(go.Scatter(
            x=d.index, y=d["ACTUAL"],
            mode="lines", name="Actual", line=dict(color="green")
        ))

        fig.add_trace(go.Scatter(
            x=d.index, y=d["FORECAST"],
            mode="lines", name="IEX Forecast", line=dict(color="pink")
        ))

        # SARIMA fitted
        fig.add_trace(go.Scatter(
            x=d.index, y=s_fit.fittedvalues,
            mode="lines", name="SARIMA Fit", line=dict(color="blue")
        ))

        # Prophet fitted
        fig.add_trace(go.Scatter(
            x=d.index, y=p_fc["yhat"].iloc[:len(d)],
            mode="lines", name="Prophet Fit", line=dict(color="red")
        ))

        # SARIMA forecast plot
        future_index = pd.date_range(start=d.index[-1], periods=97, freq="15min")[1:]
        fig.add_trace(go.Scatter(
            x=future_index, y=sarima_fc,
            mode="lines", name="SARIMA Forecast", line=dict(color="blue", dash="dash")
        ))

        # Prophet future forecast
        fig.add_trace(go.Scatter(
            x=future_p["ds"].iloc[len(d):], 
            y=p_fc["yhat"].iloc[len(d):],
            mode="lines", name="Prophet Forecast", line=dict(color="red", dash="dash")
        ))

        title_sel = day_selector.value if view_mode.value=="Day" else week_selector.value
        fig.update_layout(
            title=f"Model Comparison — {view_mode.value}: {title_sel}",
